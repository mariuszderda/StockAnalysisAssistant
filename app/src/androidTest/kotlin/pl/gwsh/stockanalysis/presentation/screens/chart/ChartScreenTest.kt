...
class ChartScreenTest {
    ...
    fun error_state_shows_error_message() {
        rule.setContent {
            ChartContent(
                state = ChartUiState.Error("AAPL", Range.ONE_MONTH, ChartType.CANDLE, false, DataError.Network),
                onBack = {}, onRefresh = {}, onToggleFavorite = {},
                onRangeChange = {}, onTypeChange = {},
            )
        }
    ...
    fun favorite_button_click_invokes_callback() {
        ...
        rule.setContent {
            ChartContent(
                state = ChartUiState.Success("AAPL", Range.ONE_MONTH, ChartType.CANDLE, false, listOf(candle(1))),
    ...
    fun range_chip_click_invokes_callback_with_new_range() {
        ...
        rule.setContent {
            ChartContent(
                state = ChartUiState.Success("AAPL", Range.ONE_MONTH, ChartType.CANDLE, false, listOf(candle(1))),
                onBack = {}, onRefresh = {}, onToggleFavorite = {},
                onRangeChange = { picked = it }, onTypeChange = {},
            )
        }
